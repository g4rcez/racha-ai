...
export const RadioGroup = <const T extends readonly Value[]>({ title, ariaLabel, values, ...props }: Props<T>) => {
    return (
        <fieldset className="block w-full">
            <label className="mb-2 block text-sm">{title}</label>
            <Radio.Root {...(props as any)} className="flex gap-4" aria-label={ariaLabel}>
                {values.map((item) => (
