...
const variants = cva(
  "flex flex-col gap-2 p-4 rounded border border-transparent w-full",
  {
    variants: {
      theme: {
...
export type AlertProps<T extends React.ElementType = "button"> =
  PolymorphicProps<Variants & Partial<{ icon: Label; title: Label }>, T>;
...
export const Alert: <T extends React.ElementType = "div">(
  ...
  return (
    <Polymorph
      ...
      as={props.as ?? "div"}
      ref={ref}
      className={css(variants({ theme, className }))}
    >
      {icon || title ? (
