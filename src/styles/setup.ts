...
export const createFromFlatToken = (key: string, val: string) => {
    ...
    return Is.undefined(second) ? `--${prefix}: ${val}` : `--${prefix}-${second.replace(prefix, "")}: ${val}`;
};

export const createColors = (value: Record<string, any>) =>
    flatTuples<string, VariableValue>(value).map(([key, val]): string => createFromFlatToken(key, val as string));
