...
export type CartUser = User & { amount: number; products: number };
...
export const Cart = Entity.create(
    ...
    (storage?: any): State => {
        return {
            ...
            products: new Dict<string, CartProduct>(storage?.products.map((x: CartProduct) => [x.id, x])),
        };
    },
    (get) => {
        ...
        return {
            ...
            addProduct: (product: CartProduct) => merge({products: get.state().products.set(product.id, product)}),
            removeProduct: (product: CartProduct) => merge({products: get.state().products.remove(product.id)}),
    ...
    },
    {
        newProduct: (consumers: Dict<string, User>): CartProduct => ({
            ...Product.create(),
            consumers: new Dict(consumers.map((x): [string, CartUser] => [x.id, {...x, amount: 0, products: 0}]))
        })
    }
);
