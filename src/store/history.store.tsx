...
const parse = (item: ParseToRaw<HistoryItem>): HistoryItem => ({
    ...
    products: Dict.from(
        "id",
        item.products.map((x) => ({ ...x, consumers: Dict.from("id", x.consumers) }))
    ),
...
const parseFromCart = (ownerId: string, cart: CartState): HistoryItem => {
    const { additional, couvert, total, products: sumProducts } = CartMath.calculate(cart);
    ...
    const users = cart.users.toArray().map((user) => {
        ...
        const ownProducts = products.reduce<HistoryProduct[]>((acc, p) => {
            ...
            return i === undefined
                ? acc
                : [...acc, { ...p, quantity: i.quantity, total: i.quantity * p.price, createdAt: p.createdAt }];
        }, [] as HistoryProduct[]);
        return {
            ...user,
            result,
    ...
    return {
        ...
        users: Dict.from(
            "id",
            users.toSorted((a, b) => (a.id === ownerId || b.id === ownerId ? 1 : b.sum - a.sum))
        )
    };
};
...
export const History = Entity.create(
    ...
    (args) => ({
        ...
        save: (ownerId: string, cart: CartState) => {
            const storage = (LocalStorage.get(args.storageKey) as { items: HistoryItem[] }) || { items: [] };
        ...
        get: (id: string): HistoryItem | null => {
            const storage = (LocalStorage.get(args.storageKey) as { items: ParseToRaw<HistoryItem>[] }) || null;
