...
const order = <T extends { name: string; id: string }>(users: T[]): T[] => users.toSorted((a, b) => (a.name.toLocaleLowerCase() > b.name.toLocaleLowerCase() ? 1 : -1));

export const Friends = Store.create(
    { name: "friends", schemas, version: "v1" },
    (store?: ParseToRaw<State>) => ({ users: Dict.from("id", store?.users ?? []) }) as State,
    (get) => ({
    ...
    }),
    {
        ...
        hasUser: (userId: string, users: User[]) => new Linq(users).Some("name", "===", userId),
