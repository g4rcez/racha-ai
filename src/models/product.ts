...
export namespace Product {
  ...
  export const isTipOrCouvert = (product: DB.OrderItem) =>
    product.productId === tipId || product.productId === couvertId;

  export const formatQuantity = (
    quantity: string,
    product: DB.OrderItemMetadata,
  ): string => {
    if (product.splitType === DivisionType.Equality) {
      return i18n.format.percent(
  ...
  export type ProductCalculus = (args: {
    quantity: number;
    price: number;
    users: Consumer[];
  }) => Consumer[];
