...
>(
  ...
  const fetchLanguage = async (newLanguage: Languages, extraOptions?: PolyglotConfig) => {
    ...
    if (Is.function(result)) return { format: formatters, map: result(formatters) };
  ...
  const createLanguage = async (
    ...
    if (newLanguage !== initialConfig.language && !Is.keyof(translations || {}, newLanguage as string)) {
  ...
  const get = <V extends keyof LanguageMap>(
    ...
    if (options?.fallback) {
      ...
      return parse(Is.function(prop) ? prop(vars, options) : prop, vars ?? {}, config.format);
    }
