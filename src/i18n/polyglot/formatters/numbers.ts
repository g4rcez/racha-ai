import {
  LocaleCurrencyCode,
  localeCurrencyMap,
} from "~/i18n/polyglot/types/currency";
...
export namespace Numbers {
  const getCountryCode = (localeString: string): LocaleCurrencyCode => {
    ...
    if (components.length === 2)
      return components.pop()?.toUpperCase() as LocaleCurrencyCode;
    const parts = localeString.split("-");
    if (parts.length === 2)
      return parts.pop()?.toUpperCase() as LocaleCurrencyCode;
  ...
  export const unitProxyHandler =
    (lang: string, options?: PolyglotConfig) =>
    (target: any, p: string | symbol) => {
      if (p in target) return target[p];
      try {
        const assign = new Intl.NumberFormat(lang, {
          unit: p as string,
          style: "unit",
          ...options?.unit,
        }).format;
